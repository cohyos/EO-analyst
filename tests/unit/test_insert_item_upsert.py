"""Unit tests for eoa.memory.relational.insert_item's F09/F36 quality-aware upsert and the
"return an inserted/conflict flag from the item upsert" efficiency item (both
SOL-AUDIT-2026-09-24).
...
# --------------------------------------------------------------------------
# Backward compatibility: callers that don't pass content_status/security_status (tenders.scan,
# the schema integration test) get the exact pre-fix "never touch content on conflict" behavior.
# --------------------------------------------------------------------------


class TestBackwardCompatibleCallers:
    def test_no_quality_params_marks_quality_aware_false(self, monkeypatch):
        cursor = _FakeCursor([{"id": 101, "inserted": True}])
        ...
        assert len(cursor.calls) == 1  # no canonical_url -> no extra lookup query
    ...
    def test_returned_value_works_anywhere_a_plain_int_id_was_expected(self, monkeypatch):
        ...
        cursor = _FakeCursor([{"id": 9, "inserted": True}])
        _patch_connection(monkeypatch, cursor)
...
# --------------------------------------------------------------------------
# F09: quality-aware content_status/security_status passed through when a caller opts in.
# --------------------------------------------------------------------------


class TestQualityAwareParams:
    def test_content_and_security_status_passed_through(self, monkeypatch):
        cursor = _FakeCursor([{"id": 5, "inserted": True}])
        _patch_connection(monkeypatch, cursor)
    ...
    def test_upsert_sql_never_downgrades_on_a_same_or_worse_fetch(self, monkeypatch):
        """The quality-rank CASE expression must appear in the SET clause for every
        content-bearing column, guarded by `quality_aware` -- a static check that the refresh
        logic is actually wired into the query text (the DB-level behavior itself needs a live
        Postgres to fully exercise; the `xmax = 0` upsert idiom and CASE-guarded SET are the two
        load-bearing pieces asserted here)."""
        cursor = _FakeCursor([{"id": 5, "inserted": True}])
        _patch_connection(monkeypatch, cursor)
...
class TestCanonicalUrlIdentity:
    def test_same_canonical_url_as_observed_url_skips_the_lookup(self, monkeypatch):
        cursor = _FakeCursor([{"id": 1, "inserted": True}])
        _patch_connection(monkeypatch, cursor)

        insert_item(source_id=None, url="https://example.com/e", canonical_url="https://example.com/e")

        assert len(cursor.calls) == 1  # straight to the ON CONFLICT (url) upsert
    ...
    def test_different_canonical_url_with_no_existing_match_falls_back_to_plain_upsert(self, monkeypatch):
        ...
        assert result.inserted is True
        assert len(cursor.calls) == 2
        assert "ON CONFLICT (url) DO UPDATE SET" in cursor.calls[1][0]

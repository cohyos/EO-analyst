...
class TestActToolCalls:
    ...
    def test_act_search_tool_call(self, monkeypatch):
        """_act() delegates search tool calls."""
        inv = Investigation(job_id=None, item_id=None, question="test")
        budget = Budget(
            max_queries=10,
            max_pages=20,
        ...
        def mock_search(query, lang, **kw):
            search_calls.append((query, lang))
            return MagicMock(hits=[], error=None)

        # First call returns search, second returns no tool calls (finish)
        mock_chat_1 = MagicMock()
        mock_chat_1.tool_calls = [
            {
                "function": {
                    "name": "search",
                    "arguments": json.dumps({"query": "test query", "lang": "en"}),
                }
            }
        ]
        mock_chat_1.content = ""
        ...
        mock_chat_2.tool_calls = []
        mock_chat_2.content = "Finishing up"

        chat_calls = [mock_chat_1, mock_chat_2]
        call_count = [0]

        def mock_chat(*a, **kw):
            result = chat_calls[min(call_count[0], len(chat_calls) - 1)]
            call_count[0] += 1
            return result

        monkeypatch.setattr("eoa.search.deep_search.chat", mock_chat)
        monkeypatch.setattr("eoa.search.deep_search._tool_search", lambda *a, **kw: "{}")
        monkeypatch.setattr("eoa.search.deep_search._check_stop", lambda *a: None)

        _act(inv, budget, [], round_no=1, max_steps=5)
        assert budget.queries == 1

    def test_act_unknown_tool_returns_error(self, monkeypatch):
        """_act() returns error for unknown tool name."""
        inv = Investigation(job_id=None, item_id=None, question="test")
        budget = Budget(
            max_queries=10,
            max_pages=20,
        ...
        mock_result2.content = "done"

        chat_calls = [mock_result, mock_result2]
        call_count = [0]
        ...
        _act(inv, budget, [], round_no=1, max_steps=5)
        # Should have recorded an error response for the tool call

    def test_act_json_parse_error_in_arguments(self, monkeypatch):
        """_act() handles malformed JSON in tool arguments."""
        inv = Investigation(job_id=None, item_id=None, question="test")
        budget = Budget(
            max_queries=10,
            max_pages=20,
        ...
        _act(inv, budget, [], round_no=1, max_steps=5)
        # Should have handled the parse error gracefully

    def test_act_invalid_finish_payload(self, monkeypatch):
        """_act() rejects invalid finish payload and continues."""
        inv = Investigation(job_id=None, item_id=None, question="test")
        budget = Budget(
            max_queries=10,
            max_pages=20,
        ...
        result = _act(inv, budget, [], round_no=1, max_steps=5)
        assert result is False  # Did not finish due to invalid payload

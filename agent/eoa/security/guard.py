...
def _l1_pipeline() -> Any:
    ...
    with _L1_LOCK:
        ...
        try:
            ...
            _L1_PIPE = pipeline("text-classification", model=spec.hf, truncation=True, max_length=512, device=-1)
...
def _l2_judge(text: str, item_id: int | str, hits: list[str]) -> dict[str, Any] | None:
    ...
    try:
        ...
        prompt = render("guard_l2", hits="; ".join(hits)[:800], data=wrap_data(text[:12_000], item_id, "guard"))
        verdict = chat_structured(role, GuardVerdict, [
...
def screen(
    ...
    *,
    ...
    if heur.score >= 0.8 or (l1 is not None and l1 >= 0.95):
        ...
        return ScreenResult("quarantined", max(heur.score, l1 or 0.0), "l1" if (l1 or 0) >= 0.95 else "heuristic",
                            kind="instruction_override", excerpt=excerpt, heuristics=heur, l1_score=l1,
    ...
    if l2 is None:
        ...
        return ScreenResult("flagged", max(heur.score, l1 or 0.0), "heuristic", kind="other",
                            excerpt=heur.hits[0].excerpt if heur.hits else "", heuristics=heur, l1_score=l1,
                            sanitizer_flags=flags)
    if l2.get("injection") and float(l2.get("confidence", 0)) >= 0.6:
        return ScreenResult("quarantined", float(l2["confidence"]), "l2", kind=str(l2.get("kind", "other")),
                            excerpt=str(l2.get("excerpt", ""))[:300], heuristics=heur, l1_score=l1, l2=l2,
                            sanitizer_flags=flags)
    return ScreenResult("clean", float(l2.get("confidence", 0.0)), "l2", heuristics=heur, l1_score=l1, l2=l2,
                        sanitizer_flags=flags)
...
def screen_and_record(item: dict[str, Any], **kw: Any) -> ScreenResult:
    ...
    if res.verdict != "clean":
        log_security(item_id=item["id"], source_id=item.get("source_id"), layer=res.layer, verdict=res.kind,
                     score=res.score, excerpt=res.excerpt, action="quarantined" if res.verdict == "quarantined" else "flagged")
    ...
    try:
        ...
        with connection() as conn:
            row = conn.execute(
                ...
                "AND created_at > now() - interval '30 days'", (source_id,)).fetchone()
            if row and row["c"] >= n:
